...
module Test.Ouroboros.Network.TipSample.TipRegistry (tests) where
...
tipRegistryProperty (TestTipFragments af a b) = do
    ...
    (pureResults a b ===) <$> getPeerResults tipRegistry
  where

    registerTips :: TipFragment BlockHeader
                 -> TipFragmentCallback Int BlockHeader m
                 -> m ()
    registerTips tf (TipFragmentCallback fn) =
    ...
    pureResults :: TipFragment BlockHeader
                -> TipFragment BlockHeader
                -> [(Down Int, Int)]
    pureResults x y = go (Down 0) (Down 0) (filterValidTips x) (filterValidTips y)
      where
        ...
        go :: Down Int -> Down Int
           -> TipFragment BlockHeader
           -> TipFragment BlockHeader
           -> [(Down Int, Int)]

        go !r1 !r2 Empty Empty =
            filter (\(Down n, _) -> n > 0) $ sort [(r1, 1), (r2, 2)]
        ...
        go !r1 !r2 tf1@(tt1 :< tf1') tf2@(tt2 :< tf2') =
            case getTipSlotNo (timedData tt1) `compare` getTipSlotNo (timedData tt2) of
              ...
              LT -> go (succ r1) r2 tf1' tf2
              GT -> go r1 (succ r2) tf1  tf2'

        go !r1  r2 (_ :< tf1') tf2@Empty   = go (succ r1) r2 tf1' tf2

        go  r1 !r2 tf1@Empty   (_ :< tf2') = go r1 (succ r2) tf1  tf2'
...
unregisterPeerProperty (TestTipFragments af a b) = do
  ...
  where
    isValid :: [(Down Int, Int)] -> Bool
    isValid = isNothing . find ((== 2) . snd)
