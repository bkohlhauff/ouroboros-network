  ...
  where
    ...
    testConfigB = TestConfigB
      ...
      , txGenExtra   = ShelleyTxGenExtra
        ...
        , stgeGenerateUpdates = not includingDUpdateTx
            -- We don't generate any other updates, since doing so might
            -- accidentally supplant the bespoke update that these tests are
            -- expecting.
            --
            -- The transaction this test introduces causes all nodes to propose
            -- the same parameter update. It'd technically be OK if some nodes
            -- then changed their proposal to a different update, as long as at
            -- least @Quorum@-many nodes were still proposing this test's
            -- original update as of the epoch boundary. However, we keep the
            -- test simple and just avoid introducing any other proposals.
        , stgeStartAt         =
